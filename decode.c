...
void decode(dict *d, char *fname, char *op_fname) {
	FILE *fp, *op;
	char *file, *token, *ext = "jay";
	char flag = 'r'; /* Flag is reset */ 
	file = (char *) malloc(sizeof(char) * (strlen(fname) + 1));
	strcpy(file, fname);

	/* Check whether the file has a .jay format or not */ 
	token = strtok(file, ".");
	while(token != NULL) {  
		if(!strcmp(token, ext))
			flag = 's'; /* Flag is set */
	...
	strcpy(file, fname);
	/* Remove the ".jay" extension from the input to write to the output file */
	uint16_t len = strlen(op_fname);
	op_fname[len - 4] = '\0';

	/* Open the compressed file with format ".jay" */
	fp = d_open_file(file);
